...
#                 if not shp_files:
#                     # Try looking in a single nested subfolder if that's all there is
#                     contents = os.listdir(shp_path)
#                     subdirs = [d for d in contents if os.path.isdir(os.path.join(shp_path, d))]
#                     if len(subdirs) == 1:
#                         nested_path = os.path.join(shp_path, subdirs[0])
...
#                     print(f"✅ Loaded single shapefile: {shp_files[0]}")
#                     return result
...
#     change = load_shapefile(change_path)
...
#     # SAVE EXCEL
#     # =========================
#     excel_df = final[[main_id_col, 'geometry', 'change_percent']].copy()

#     excel_df['geometry'] = excel_df['geometry'].apply(
#         lambda g: g.wkt if g else None
#     )
#     excel_df['changed'] = final['CHANGED']
#     excel_df = excel_df.rename(columns={main_id_col: 'objectid'})
#     excel_df = excel_df[['objectid', 'geometry', 'change_percent', 'changed']]
